...
contract AuthService {
    ...
    modifier onlyManager() {
        require(
            permitManagerMapping[tx.origin] == true,
            "AuthService onlyManager：你不是授权管理者，无法进行对应操作！"
        );
        _;
    }
...
contract CarbonFrugalEvidence is AuthService {
    ...
    event authDeviceOperation(
        address indexed operaAccount,
    ...
    modifier onlyUser(address account) {
        require(
            AuthService.isAuthUser(account) == true,
            "CarbonFrugalEvidence onlyUser:必须是注册用户,才可以进行碳节能存证！"
        );
        _;
    }
    ...
    modifier onlyAuthDevice() {
        require(
            _authDevice[msg.sender] == true,
            "CarbonFrugalEvidence onlyAuthDevice：未授权设备，无法进行对应操作！"
        );
        _;
    }
    ...
    modifier signVerify(
        ...
        require(
            retAddr == account,
            "CarbonFrugalEvidence signVerify:数据签名和当前需要存证用户不一致!"
        );
        _;
    }
    ...
    function setAuthDevice(address account) public onlyManager {
        require(
            account != address(0),
            "CarbonFrugalEvidence  setAuthDevice: 授权设备为空地址！"
        );
    ...
    function changeAuthDevice(address account) public onlyManager {
        require(
            account != address(0),
            "CarbonFrugalEvidence  changeAuthDevice: 授权设备为空地址！"
        );
    ...
    function addEvidence(
        ...
        onlyUser(account)
        onlyAuthDevice
        signVerify(account, dataHash, signStr)
    {
        _evidence.add(account, dataHash, dataJson, signStr, timeCertificate);
        require(
            _evidence.has(account, dataHash),
            "CarbonFrugalEvidence addEvidence： 存证失败，存入区块链异常！"
        );
        emit EvidenceAdded(
            msg.sender,
            account,
    ...
    function selectEvidence(address account, bytes32 dataHash)
        public
        view
        returns (string)
    {
    ...
    //删除存证，授权管理者操作
    function removeEvidence(address account, bytes32 dataHash)
        public
        onlyUser(account)
        onlyManager
    {
        require(
            _evidence.has(account, dataHash),
            "CarbonFrugalEvidence removeEvidence：不存在对应的存证数据！"
        );
    ...
    function sendTransaction(
        ...
        bytes signStr,
        string timeCertificate
    ) public onlyUser(msg.sender) signVerify(msg.sender, dataHash, signStr) {
